...
.attracation_back{position:absolute; top:-36px; left:830px; border-radius: 5px;
				  ...
				  cursor:pointer}
#attracation_head{width:300px; height:50px; position:absolute; top:30px; left:50%; margin-left:-100px}
...
$(function(){
	$(".seatchoice").click(function(){
		...
		$.ajax({
			type : "POST",
			data : {"attraction_img" : data},
			url : "./attractiondetail.bo",
	...
	$(".attracation_back").click(function(){
		history.go(-1);
	});
});
</script>
...
<div id="attracation_area">
	<div id="attracation_head">
		<h2>${fn:substring(station_name,0,2)}추천 명소</h2>
	</div>
<c:forEach var="list" items="${atlist}" varStatus="status">
<table id="attable${status.count}" class="attable">
	<tr><td><h4>&nbsp;&nbsp;${list.attraction_name}</h4></td></tr>
	<tr><td><input type="image" class="seatchoice" value="${list.attraction_img}"
	src="./attraction_img/${list.attraction_img}"
	height="200" width="400" id="${list.attraction_img}"></td></tr>
	<tr><td>위치 : ${list.attraction_position}</td></tr>
	<tr><td>관련 홈페이지 : <br><a href="${list.attraction_url}">${list.attraction_url}</a></td></tr>
</table>
</c:forEach>
<input type="button" value="목록으로 돌아가기" class="attracation_back">
</div>
